...
const auditActionValidator = v.union(
  ...
  v.literal("RM_CHECK_INS_UPDATE"),
  v.literal("OPS_KPI_TARGETS_INSERT"),
  v.literal("OPS_KPI_TARGETS_UPDATE"),
  v.literal("OPS_KPI_TARGETS_DELETE"),
  v.literal("OPS_WARNINGS_INSERT"),
  v.literal("OPS_WARNINGS_UPDATE"),
  v.literal("OPS_WARNINGS_DELETE"),
  v.literal("OPS_CHECK_IN_NOTES_INSERT"),
  v.literal("OPS_CHECK_IN_NOTES_UPDATE"),
  v.literal("OPS_CHECK_IN_NOTES_DELETE"),
  v.literal("OWNER_VERIFICATIONS_INSERT"),
  v.literal("OWNER_VERIFICATIONS_UPDATE"),
  ...
  v.literal("TENANT_FAVORITES_INSERT"),
  v.literal("TENANT_FAVORITES_UPDATE"),
  v.literal("TENANT_FAVORITES_DELETE"),
  v.literal("TENANT_FAVORITE_ADD"),
  v.literal("TENANT_FAVORITE_REMOVE"),
  ...
  v.literal("NEGOTIATION_TERMS_SIGNATURES_UPDATE"),
  ...
  v.literal("TENANT_PROFILE_UPDATE"),
  v.literal("TENANT_REFERRAL_SHARE"),
  v.literal("tenant.chat_opened"),
  v.literal("tenant.message_sent"),
  v.literal("owner.message_sent"),
  v.literal("tenant.dashboard_viewed"),
  v.literal("ops_kpi_target.create"),
  v.literal("ops_kpi_target.update"),
  v.literal("ops_kpi_target.cancel"),
  v.literal("ops_kpi_target.bulk_create"),
  v.literal("ops_kpi_target.roll_forward"),
  v.literal("ops_warning.issue"),
  v.literal("ops_warning.acknowledge"),
  v.literal("ops_warning.resolve"),
  v.literal("ops_warning.escalate"),
  v.literal("ops_check_in.create"),
  v.literal("ops_check_in.update"),
  v.literal("ops_check_in.toggle_action_item"),
  v.literal("VOICE_TRANSCRIPTIONS_INSERT"),
  v.literal("VOICE_TRANSCRIPTIONS_UPDATE"),
);
const systemConfigKeyValidator = v.union(
  ...
  v.literal("warning_quality_threshold"),
  v.literal("warning_target_miss_streak"),
  v.literal("warning_sla_breach_count_30d"),
  v.literal("warning_inactivity_days"),
  v.literal("warning_level1_expiry_days"),
  v.literal("warning_level2_expiry_days"),
  v.literal("warning_escalation_auto"),
  v.literal("checkin_overdue_days"),
  v.literal("caseload_threshold"),
  ...
  v.literal("multi_persona_enabled"),
  v.literal("sha_signing_date"),
  v.literal("sha_strategic_authority_months"),
  v.literal("salary_cap_founder_two_paise"),
  v.literal("salary_cap_founder_three_paise"),
  v.literal("dividend_advance_cap_collective_paise"),
  v.literal("dividend_max_per_distribution_paise"),
  v.literal("expense_threshold_single_paise"),
  v.literal("expense_threshold_monthly_paise"),
  v.literal("compensation_threshold_annual_paise"),
  v.literal("excess_withdrawal_interest_rate_bps"),
  v.literal("interest_day_count_basis"),
  v.literal("category_b_auto_approve_business_days"),
  v.literal("finance_signatory_user_id"),
);
...
const opsKpiMetricValidator = v.union(
  v.literal("visits_completed"),
...
const opsKpiPeriodTypeValidator = v.union(
  v.literal("WEEKLY"),
  v.literal("MONTHLY"),
  v.literal("QUARTERLY"),
);
const opsKpiTargetStatusValidator = v.union(
  v.literal("ACTIVE"),
  v.literal("COMPLETED"),
  v.literal("MISSED"),
  v.literal("EXCEEDED"),
...
const opsWarningIssuedByTypeValidator = v.union(v.literal("USER"), v.literal("SYSTEM"));
const opsWarningTriggerTypeValidator = v.union(v.literal("AUTO"), v.literal("MANUAL"));
const opsWarningTriggerReasonValidator = v.union(
...
const opsWarningStatusValidator = v.union(
  v.literal("ACTIVE"),
...
const opsCheckInSentimentValidator = v.union(
  v.literal("POSITIVE"),
  v.literal("NEUTRAL"),
...
export default defineSchema({
  users: defineTable({
    ...
    user_types: v.optional(v.array(userTypeValidator)),
    active_persona: v.optional(userTypeValidator),
  })
  ...
  ops_kpi_targets: defineTable({
    agent_user_id: v.id("users"),
    set_by_user_id: v.id("users"),
    metric: opsKpiMetricValidator,
    target_value: v.number(),
    period_type: opsKpiPeriodTypeValidator,
    period_start: v.number(),
    period_end: v.number(),
    actual_value: v.optional(v.number()),
    progress_pct: v.optional(v.number()),
    status: opsKpiTargetStatusValidator,
    notes: v.optional(v.string()),
    created_at: v.number(),
    updated_at: v.number(),
    is_deleted: v.boolean(),
  })
    .index("by_agent", ["agent_user_id", "period_type", "status"])
    .index("by_period", ["period_start", "period_end"])
    .index("by_status", ["status"])
    .index("by_agent_metric_period", ["agent_user_id", "metric", "period_start"]),

  ops_warnings: defineTable({
    agent_user_id: v.id("users"),
    issued_by_user_id: v.optional(v.id("users")),
    issued_by_type: opsWarningIssuedByTypeValidator,
    warning_level: v.number(),
    trigger_type: opsWarningTriggerTypeValidator,
    trigger_reason: opsWarningTriggerReasonValidator,
    description: v.string(),
    evidence: v.optional(v.string()),
    status: opsWarningStatusValidator,
    ...
    .index("by_agent", ["agent_user_id", "status"])
    .index("by_level", ["warning_level", "status"])
    .index("by_status", ["status", "created_at"])
    .index("by_status_expires_at", ["status", "expires_at"])
    .index("by_agent_level_reason_status", [
      "agent_user_id",
      "warning_level",
      "trigger_reason",
      "status",
  ...
  ops_check_in_notes: defineTable({
    agent_user_id: v.id("users"),
    ...
    sentiment: v.optional(opsCheckInSentimentValidator),
    next_review_date: v.optional(v.number()),
    created_at: v.number(),
    updated_at: v.number(),
    is_deleted: v.boolean(),
  })
    .index("by_agent", ["agent_user_id", "created_at"])
    .index("by_reviewer", ["reviewer_user_id", "created_at"]),
  ...
  tenant_favorites: defineTable({
    ...
    .index("by_tenant_and_listing", ["tenant_user_id", "listing_id"]),
  ...
  rental_transactions: defineTable({
    ...
    source_negotiation_id: v.optional(v.id("negotiations")),
  ...
  notification_preferences: defineTable({
    ...
    .index("by_user_id", ["user_id"])
    .index("by_user_is_deleted", ["user_id", "is_deleted"])
    .index("by_is_deleted", ["is_deleted"]),
  ...
  // NOTE: Scaffolded for Phase 36+ (Monetization). No runtime operations yet.
  ...
  // NOTE: Scaffolded for gamification features. No runtime operations yet.
